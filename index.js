...
export default (option, dayjsClass, dayjsFactory) => {
  ...
  var Recur = (function () {
    ...
    function isException(exceptions, date) {
      for (var i = 0, len = exceptions.length; i < len; i++) {
        if (dayjsFactory(exceptions[i]).isSame(date)) {
          return true
        }
      }
      return false
    ...
    var Recur = function (options) {
      if (options.start) {
        this.start = dayjsFactory(options.start).dateOnly()
      }

      if (options.end) {
        this.end = dayjsFactory(options.end).dateOnly()
      }
    ...
    Recur.prototype.startDate = function (date) {
      ...
      if (date) {
        this.start = dayjsFactory(date).dateOnly()
        return this
      }
    ...
    Recur.prototype.endDate = function (date) {
      ...
      if (date) {
        this.end = dayjsFactory(date).dateOnly()
        return this
      }
    ...
    Recur.prototype.fromDate = function (date) {
      ...
      if (date) {
        this.from = dayjsFactory(date).dateOnly()
        return this
      }
    ...
    Recur.prototype.except = function (date) {
      date = dayjsFactory(date).dateOnly()
    ...
    Recur.prototype.forget = function (dateOrRule) {
      var i, len
      var whatMoment = dayjsFactory(dateOrRule)
    ...
    Recur.prototype.matches = function (dateToMatch, ignoreStartEnd) {
      var date = dayjsFactory(dateToMatch).dateOnly()
    ...
    for (var measure in measures) {
      if (ruleTypes.hasOwnProperty(measure)) {
        Recur.prototype[measure] = Recur.prototype[
          measures[measure]
  ...
  dayjsFactory.recur = function (start, end) {
    ...
    if (start === Object(start) && !dayjsFactory.isDayjs(start)) {
  ...
  dayjsClass.prototype.recur = function (start, end) {
    // If we have an object, use it as a set of options
    if (start === Object(start) && !dayjsFactory.isDayjs(start)) {
  ...
  dayjsClass.prototype.monthWeek = function () {
